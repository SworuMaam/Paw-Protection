  ...
  async (req, { params }) => {
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Failed to fetch pet", details: error.message },
        { status: 500 }
      );
    }
  },
...
export const PUT = withAuth(
  async (req: NextRequest, { params }) => {
    ...
    try {
      ...
      const foster_parent_id_raw = form
        .get("foster_parent_id")
        ?.toString()
        .trim();
      const foster_parent_id = foster_parent_id_raw
        ? Number(foster_parent_id_raw)
        : null;

      const pgTemperamentArray = `{${temperament
        .map((t) => `"${t}"`)
        .join(",")}}`;

      let location_address_raw = form
        .get("location_address")
        ?.toString()
        .trim();
      let availability_status = form
      ...
      if (!location_address && location_address_raw) {
        try {
          ...
          return NextResponse.json(
            { error: "Invalid location_address format" },
            { status: 400 }
          );
        }
      }
...
export const DELETE = withAuth(
  async (req, { params }) => {
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Failed to delete pet", details: error.message },
        { status: 500 }
      );
    }
  },
